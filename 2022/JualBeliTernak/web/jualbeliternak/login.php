...
if (isset($_POST['username']) && isset($_POST['password'])) {
	...
	$connLogin = $conn;
	$sqlPassword = "SELECT `username`, `password`, `role`  FROM `pantauternak_users` WHERE `username` = '" . $username . "'";
	...
	<script>
		...
		const validasiForm = (nilai) => {
			...
			if (!nilai.password) {
				...
				errors.password = `Password harus lebih dari ${PASSWORD_LENGTH} karakter`;
			}
