...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `kontroluv_log` WHERE `time_stamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `kontroluv_log`.`time_stamp` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `kontroluv_log` ORDER BY `kontroluv_log`.`time_stamp` DESC";
	}
...
<html>
	<head>
		<title>Kontrol Penggunaan UV dan LED</title>
		<meta charset="UTF-8">
	...
	<body class="w3-light-grey" onload="setClock()">
      ...
      <span class="w3-bar-item w3-hide-small w3-hide-medium w3-xlarge">Kontrol Penggunaan UV dan LED</span>
			  ...
			  <div>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-blue-grey'>
								...
								<th style="vertical-align: middle;">Lux Lampu LED</th>
								<th style="vertical-align: middle;">Suhu (&deg;C)</th>
								<th style="vertical-align: middle;">Kelembapan (%)</th>
								<th style="vertical-align: middle;">UV Matahari</th>
								<th style="vertical-align: middle;">UV Lampu</th>
							...
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											echo "<td>".($i+1)."</td>";
											echo "<td>".$resultsHistory->data[$i]['lux_led']."</td>";
											echo "<td>".$resultsHistory->data[$i]['temp']."</td>";
											echo "<td>".$resultsHistory->data[$i]['humid']."</td>";
											echo "<td>".$resultsHistory->data[$i]['uv_outdoor']."</td>";
											echo "<td>".$resultsHistory->data[$i]['uv_indoor']."</td>";
