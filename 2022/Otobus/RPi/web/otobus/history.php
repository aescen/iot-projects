...
<?php
  ...
  require_once 'inits.php';
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `otobus_log` WHERE `time_stamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `otobus_log`.`time_stamp` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `otobus_log` ORDER BY `otobus_log`.`time_stamp` DESC";
	}
...
<html>
	...
	<body class="w3-light-grey" onload="setClock()">
      ...
      <span class="w3-bar-item w3-hide-small w3-hide-medium w3-xlarge"><strong>Otobus</strong></span>
		  ...
		  <div class="w3-panel">
			<div class="w3-row-padding w3-container" style="margin:0 -16px">
			  <div>
				...
				<br>
				<table class="w3-table-all w3-centered w3-white">
							<tr class='w3-teal'>
							...
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											echo "<td>".($i+1)."</td>";
											echo "<td>".$resultsHistory->data[$i]['power_usage']."w</td>";
                      echo "<td>".$resultsHistory->data[$i]['node_id']."</td>";
                      echo "<td>".NODE_INFO[$resultsHistory->data[$i]['info']]."</td>";
