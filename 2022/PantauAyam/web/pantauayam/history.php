...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `pantauayam_log` WHERE `ayam_mati` > 0 AND `time_stamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `pantauayam_log`.`time_stamp` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `pantauayam_log` WHERE `ayam_mati` > 0 ORDER BY `pantauayam_log`.`time_stamp` DESC";
	}
...
<html>
	<head>
		<title>Pantau Ayam History</title>
		<meta charset="UTF-8">
			  ...
			  <div>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-khaki'>
								...
								<th style="vertical-align: middle;">Ayam Mati</th>
							...
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											echo "<td>".($i+1)."</td>";
											echo "<td>".$resultsHistory->data[$i]['ayam_mati']."</td>";
