...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `pantaujeruk_log` WHERE `time_stamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `pantaujeruk_log`.`time_stamp` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `pantaujeruk_log` ORDER BY `pantaujeruk_log`.`time_stamp` DESC";
	}
								...
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											...
											echo "<td>".$resultsHistory->data[$i]['moist']."</td>";
											echo "<td>".$resultsHistory->data[$i]['ph']."</td>";
