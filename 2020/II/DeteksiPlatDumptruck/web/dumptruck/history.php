...
<?php		
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT ids, plateNumbers AS Plat, comeIn AS Masuk, comeOut AS Keluar FROM `platenumbershistory` WHERE `comeIn` BETWEEN '$startTime' AND '$endTime' ORDER BY `ids` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT ids, plateNumbers AS Plat, comeIn AS Masuk, comeOut AS Keluar FROM `platenumbershistory`";
	}
	...
	<body class="w3-light-grey" onload="setClock()">
		...
		<nav class="w3-sidebar w3-bar-block w3-collapse w3-white w3-animate-left" style="z-index:3;width:140px;" id="mySidebar"><br>
		  ...
		  <div class="w3-bar-block">
			...
			<a href="platnomor.php" class="w3-bar-item w3-button w3-padding"><i class="fa fa-save fa-fw"></i>  Plat nomor</a>
		  </div>
		</nav>
			...
			<div class="w3-row-padding w3-container" style="margin:0 -16px">
			  <div>
				...
				<div class="w3-cell-row">
					...
					<?php	if (isset($resultsHistory->total)){
								echo "<div class='w3-right-align w3-cell w3-cell-middle w3-mobile'><strong>Total data: ".$resultsHistory->total."</strong></div>";
							}
					?>
				</div>
				<br>
				<table class="w3-table w3-card w3-bordered w3-striped w3-white w3-centered">
							...
							<?php	if ($PaginatorHistory->getNumRows() > 0) {
										for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
											echo "<tr>";
												echo "<td>".($i+1)."</td>";
												echo "<td>".$resultsHistory->data[$i]['Plat']."</td>";
												echo "<td>".(!isset($resultsHistory->data[$i]['Masuk']) ? "-" : $resultsHistory->data[$i]['Masuk'])."</td>";
												echo "<td>".(!isset($resultsHistory->data[$i]['Keluar']) ? "-" : $resultsHistory->data[$i]['Keluar'])."</td>";
											echo "</tr>";
										endfor; 
									}
									else{
		...
		<script type="text/javascript">
			function setClock () {
				...
				var currentDate = currentTime.getDate().toString();
				if (currentDate.length === 1) currentDate = "0" + currentDate;
				var currentTimeString = currentYear + "-" + currentMonth + "-" + currentDate;
				var currentTimeString2 = currentDate + "/" + currentMonth + "/" + currentYear;
				const ds = document.getElementById('dateselect');
				ds.setAttribute('max' , currentTimeString);
			}
		</script>
	</body>
</html>