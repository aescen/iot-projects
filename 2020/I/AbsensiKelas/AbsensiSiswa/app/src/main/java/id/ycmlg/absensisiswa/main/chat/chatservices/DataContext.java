...
public class DataContext extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String tblFriends = "create table if not exists Friends (userid text, Email text, FirstName text, LastName text);";
    ...
    public List<User> getUserFriendList() {
        ...
        while (!c.isAfterLast()) {
            try {
                ...
                friend.id = c.getString(c.getColumnIndex("userid"));
                //friend.uid = c.getString(c.getColumnIndex("uid"));
        ...
        Collections.sort(friendList, (o1, o2) -> o1.FirstName.compareTo(o2.FirstName));
    ...
    public void refreshUserFriendList(List<User> friendList) {
        ...
        for (User item : friendList) {
            ...
            if (checkFriendAlreadyExists(item.Email) == 0) {
                ...
                String query = "insert into Friends (userid, Email, FirstName, LastName) values('" + item.id + "','" + item.Email + "', '" + item.FirstName + "', '" + item.LastName + "');";
                db.execSQL(query);
                // db.close();
            }
        }
    ...
    public User getFriendByEmailFromLocalDB(String friendEmail) {
        ...
        try{
            ...
            if (c.getCount() > 0 && c.getString(c.getColumnIndex("Email")).contentEquals(friendEmail)) {
                friend.id = c.getString(c.getColumnIndex("userid"));
                //friend.uid = c.getString(c.getColumnIndex("uid"));
                friend.Email = c.getString(c.getColumnIndex("Email"));
                friend.FirstName = c.getString(c.getColumnIndex("FirstName"));
    ...
    public void saveMessageOnLocalDB(String from, String to, String message, String sentDate) {
        SQLiteDatabase db = getWritableDatabase();
    ...
    public List<Message> getUserLastChatList(String userMail) {
        ...
        if(c != null) c.close();
        Collections.sort(userLastChat, (o1, o2) -> {
