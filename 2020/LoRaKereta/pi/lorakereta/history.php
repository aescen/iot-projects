...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `loratrainlog` WHERE `timestamps` BETWEEN '$startTime' AND '$endTime' ORDER BY `id` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `loratrainlog`";
	}
			  ...
			  <div>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-blue-grey'>
								...
								<th style="vertical-align: middle;" rowspan="2">Arah</th>
								<th style="vertical-align: middle;" rowspan="2">Lampu</th>
								<th style="vertical-align: middle;" rowspan="2">Timestamp</th>
							</tr><tr></tr>
							<?php
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										if ( isset($resultsHistory->data[$i]['directionlamp']) ||
											 !ctype_space($resultsHistory->data[$i]['directionlamp']) ||
											 $resultsHistory->data[$i]['directionlamp'] != "" ||
											 isset($resultsHistory->data[$i]['stopgolamp']) ||
											 !ctype_space($resultsHistory->data[$i]['stopgolamp']) ||
											 $resultsHistory->data[$i]['stopgolamp'] != ""){
												$resultsHistory->data[$i]['directionlamp'] = $resultsHistory->data[$i]['directionlamp'] == "FromLeft" ? "Dari kiri" : "Dari kanan";
												$resultsHistory->data[$i]['stopgolamp'] = $resultsHistory->data[$i]['stopgolamp'] == "Stop" ? "Berhenti" : "Jalan";
												echo "<tr>";
													echo "<td>".($i+1)."</td>";
													echo "<td>".$resultsHistory->data[$i]['traincount']."</td>";
													echo "<td>".$resultsHistory->data[$i]['directionlamp']."</td>";
													echo "<td>".$resultsHistory->data[$i]['stopgolamp']."</td>";
													echo "<td>".$resultsHistory->data[$i]['timestamps']."</td>";
												echo "</tr>";
										}
									endfor;
								}
