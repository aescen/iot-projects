...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `palangcovlog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `id` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `palangcovlog`";
	}
			  ...
			  <div>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-blue-grey'>
								...
								<th style="vertical-align: middle;">Object Temperature</th>
								<th style="vertical-align: middle;">Ambient Temperature</th>
							...
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									...
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											echo "<td>".($i+1)."</td>";
											echo "<td>".$resultsHistory->data[$i]['objTemp']."</td>";
											echo "<td>".$resultsHistory->data[$i]['ambTemp']."</td>";
											echo "<td>".$resultsHistory->data[$i]['humidDHT']."</td>";
											echo "<td>".$resultsHistory->data[$i]['tempDHT']."</td>";
