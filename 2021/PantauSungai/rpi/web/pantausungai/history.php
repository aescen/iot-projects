...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `pantausungailog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `pantausungailog`.`timeStamp` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `pantausungailog` ORDER BY `pantausungailog`.`timeStamp` DESC";
	}
...
<html>
	<head>
		<title>Pantau Sungai History</title>
		<meta charset="UTF-8">
	...
	<body class="w3-light-grey" onload="setClock()">
		...
		<div class="w3-bar w3-top w3-black w3-large" style="z-index:4">
          ...
          <span class="w3-bar-item w3-hide-small w3-left w3-xlarge">Pantau Sungai</span>
			  ...
			  <div>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-blue-grey'>
								...
								<th style="vertical-align: middle;">Sensor Type</th>
								<th style="vertical-align: middle;">Sensor Value</th>
							...
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											echo "<td>".($i+1)."</td>";
											echo "<td>".$resultsHistory->data[$i]['sensorType']."</td>";
											echo "<td>".$resultsHistory->data[$i]['sensorVal']."%</td>";
											echo "<td>".$arrStrStatus[strval($resultsHistory->data[$i]['objectStatus'])]."</td>";
