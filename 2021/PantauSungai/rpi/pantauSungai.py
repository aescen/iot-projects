...
class VideoCannyProcessing:
    ...
    def cannyForward(self, frame):
        ...
        for spot in range(0, len(self.listROI)):
            (x1, y1, x2, y2) = self.listROI[spot]
            new = frame[y1:y2, x1:x2, :]
            #cv2.imwrite('./output/images/output/new' + str(self.it) + '_' + str(spot) + '.jpg', new)

            # extract Canny edges
            newEdges = cv2.Canny(new, self.cannyTresh1, self.cannyTresh2)
            #cv2.imwrite('./output/images/outputcanny/newCanny' + str(self.it) + '_'  + str(spot) + '.jpg', newEdges)
            ...
            if cannySum > threshold:
                ...
                # draw a green rectangle to indicate a bersih ROI spot
                ...
                self.spotsStatus[spot].update({'spot': spot,
                                              ...
                                              'cannysum': cannySum,
                                              'threshold': threshold})
            #cv2.imwrite('./output/images/output/newFrame' + str(self.it) + '_' + str(spot) + '.jpg', frame)
            #cv2.imwrite('./output/images/outputcanny/newFrameCanny' + str(self.it) + '_'  + str(spot) + '.jpg', frameCanny)
    ...
    def loadTemplate(self):
        ...
        for r in self.listROI:  # proses canny
            ...
            #cv2.imwrite('./output/images/output/newTemplate' + str(r) + '.jpg', template)
    ...
    def start(self):
        ...
        self.outvideo = cv2.VideoWriter('./output/videos/' + str(datetime.datetime.now().strftime('%Y-%m-%d_%H.%M.%S')) +
                   '_output.avi',fourcc, 60.0, (width, height)) # 60.0 fps
