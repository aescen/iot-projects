...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `accontrollog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `accontrollog`.`timeStamp` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `accontrollog` ORDER BY `accontrollog`.`timeStamp` DESC";
	}
...
<html>
	<head>
		<title>AC Control History</title>
		<meta charset="UTF-8">
			  ...
			  <div>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-blue-grey'>
								...
								<th style="vertical-align: middle;">Object Count</th>
								<th style="vertical-align: middle;">Room Temperature</th>
								<th style="vertical-align: middle;">Humidity</th>
								<th style="vertical-align: middle;">AC Temperature</th>
								<th style="vertical-align: middle;">Total Visitor</th>
							...
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											echo "<td>".($i+1)."</td>";
											echo "<td>".$resultsHistory->data[$i]['objectCount']."</td>";
											echo "<td>".$resultsHistory->data[$i]['roomTemp']."</td>";
											echo "<td>".$resultsHistory->data[$i]['humidity']."</td>";
											echo "<td>".$resultsHistory->data[$i]['acTemp']."</td>";
											echo "<td>".$resultsHistory->data[$i]['totalVisitor']."</td>";
