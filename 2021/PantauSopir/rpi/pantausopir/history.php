...
if (isset($dateSelect)) {
  ...
  $queryHistory    = "SELECT * FROM `pantausopirlog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `pantausopirlog`.`timeStamp` DESC";

  $ymd = date("Y-m-d", strtotime($dateSelect));
  $_SESSION['ymd'] = $ymd;
} else {
  $queryHistory    = "SELECT * FROM `pantausopirlog` ORDER BY `pantausopirlog`.`timeStamp` DESC";
}
...
<head>
  <title>Pantau Sopir History</title>
  <meta charset="UTF-8">
...
<body class="w3-light-grey">
  ...
  <div class="w3-bar w3-top w3-black w3-large" style="z-index:4">
    ...
    <span class="w3-bar-item w3-hide-small w3-left w3-xlarge">Pantau Sopir</span>
    ...
    <div class="w3-panel">
      <div class="w3-row-padding w3-container" style="margin:0 -16px">
        <div>
          ...
          <br>
          <div class="img-view">
            <table class="w3-table-all w3-centered w3-card-4 w3-white w3-center">
              <tr class='w3-blue-grey'>
                <th style="vertical-align: middle;">No</th>
                <th style="vertical-align: middle;">BPM</th>
                <th style="vertical-align: middle;">Oxy</th>
              ...
              if ($PaginatorHistory->getNumRows() > 0) {
                for ($i = 0; $i < count($resultsHistory->data); $i++) :
                  ...
                  echo "<td style='vertical-align: middle;'>" . $resultsHistory->data[$i]['bpmVal'] . "</td>";
                  echo "<td style='vertical-align: middle;'>" . $resultsHistory->data[$i]['oxyVal'] . "%</td>";
                  /* echo "<td style='vertical-align: middle;'>" . $resultsHistory->data[$i]['nodeId'] . "</td>"; */
                  echo "<td style='vertical-align: middle;'>" . $resultsHistory->data[$i]['timeStamp'] . "</td>";
                  echo "<td style='vertical-align: middle;'><a href=#'><img id='" . $resultsHistory->data[$i]['imgPath'] .
                    "'src='./imgs/" . $resultsHistory->data[$i]['imgPath'] .
                    ".jpg' alt='captured-img-" . $resultsHistory->data[$i]['imgPath'] .
