...
const char* serverUrl PROGMEM = "http://192.168.1.5/pantaukolam/api/index.php?setTable=kolam&";
const char* updatePhBGetUri PROGMEM = "valPhB=";
...
void updateDb() {
  WiFiClient client;
  HTTPClient http;
  const String updateData = (String)serverUrl + (String)updatePhBGetUri + (String)valPhB +
...
void loop() {
  float newValPhB = random(44, 78);
  float newValTurbidity = random(90, 99);
  if (WiFi.status() == WL_CONNECTED) {
    if ((millis() - updateTimer) > updateDelay) {
      if (newValPhB != valPhB || newValTurbidity != valTurbidity) {
        valPhB = newValPhB;
