...
def main():
    ...
    def control():
        stopcontrol = False
        def start():
            global stopcontrol, weight, weightThresh, relayPin, newDetect1, newDetect2
            tmp = False
            while True:
                try:
                    if not stopcontrol:
        ...
        nT = Thread(target=start, args=(), name='IO_Control')
        nT.start()

    def cleanup():
        global newDetect1, newDetect2, newWeight, newControl
        newDetect1.stop()
        newDetect2.stop()
        newWeight.stop()
        newControl.stopcontrol = True
        tracemalloc.stop()
    ...
    try:
        ...
        while True:
            ...
            screen.refresh()
            screen.addstr(3, 0, "\r[Memory usage] Current: %fMB - Peak: %fMB" % (memCurrent / 10**6, memPeak / 10**6))
        ...
        while True:
            try:
                if self.stopped:
                    ...
                    self.value = max(0, int(self.hx.get_weight(self.sample) * 1000)) / 1000
                    self.hx.power_down()
                    self.hx.power_up()
                    time.sleep(0.1)
            except KeyboardInterrupt:
