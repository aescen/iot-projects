...
<?php
	...
	if(isset($_POST['dateselect']) && isset($_POST['tipetelur'])){
        $_SESSION['dateselect'] = $_POST['dateselect'];
        $_SESSION['tipetelur'] = $_POST['tipetelur'];
    }
    
    if(isset($_SESSION['dateselect']) && isset($_SESSION['tipetelur'])){
        $dateSelect = $_SESSION['dateselect'];
        $tipeTelur = $_SESSION['tipetelur'];
    }
    else{
        unset($dateSelect);
	...
	if( isset($dateSelect) ){
		...
		if ($tipeTelur == '1'){
			$queryHistory      = "SELECT * FROM `kualitastelurlog` WHERE `kualitastelurlog`.`tipeTelur` = 'Segar' AND `kualitastelurlog`.`Waktu` BETWEEN '$startTime' AND '$endTime' ORDER BY `id` DESC";
		} else if ($tipeTelur == '2'){
			$queryHistory      = "SELECT * FROM `kualitastelurlog` WHERE `kualitastelurlog`.`tipeTelur` = 'BerkembangBiak' AND `kualitastelurlog`.`Waktu` BETWEEN '$startTime' AND '$endTime' ORDER BY `id` DESC";
		} else if ($tipeTelur == '3'){
			$queryHistory      = "SELECT * FROM `kualitastelurlog` WHERE `kualitastelurlog`.`tipeTelur` = 'Busuk' AND `kualitastelurlog`.`Waktu` BETWEEN '$startTime' AND '$endTime' ORDER BY `id` DESC";
		} else {
			$queryHistory      = "SELECT * FROM `kualitastelurlog` WHERE `kualitastelurlog`.`Waktu` BETWEEN '$startTime' AND '$endTime' ORDER BY `id` DESC";
		}
		...
		$queryHistory      = "SELECT * FROM `kualitastelurlog` ORDER BY `id` DESC";
	}
	...
	<body class="w3-light-grey" onload="setClock()">
		...
		<div class="w3-main w3-light-grey" style="margin-left:140px;margin-top:43px;">
		  ...
		  <header class="w3-container" style="padding-top:22px">
			<div class="w3-container w3-blue-grey">
				<h2>History <?php echo ($tipeTelur == 0)? '(Semua Telur)': '(Telur ' . $tipeTelur . ')'; ?> </h2>
			  </div>
		  </header>
			  ...
			  <div>
				...
				<div class="w3-cell-row">
					<div class="w3-left-align">
						<form action="history.php" method="post" onsubmit="return valDate()" required>
							<label for="tipetelur">Tipe</label>
							<select id="tipetelur" name="tipetelur" autofocus>
							 <option value="0" <?php echo ($tipeTelur == 0)? 'selected': ''; ?> >Semua</option>
							 <option value="1" <?php echo ($tipeTelur == 1)? 'selected': ''; ?> >Segar</option>
							 <option value="2" <?php echo ($tipeTelur == 2)? 'selected': ''; ?> >Berkembang biak</option>
							 <option value="3" <?php echo ($tipeTelur == 3)? 'selected': ''; ?> >Busuk</option>
							</select>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-blue-grey'>
                                ...
                                <th style="vertical-align: middle;">Tipe Telur</th>
                                <th style="vertical-align: middle;">Beban</th>
							...
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									...
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
										echo "<tr>";
											echo "<td>" . ($i+1) . "</td>";
											echo "<td>" . strtoupper($resultsHistory->data[$i]['tipeTelur']) . "</td>";
											echo "<td>" . $resultsHistory->data[$i]['beban'] . "</td>";
