...
$sqlTurbidityChart =
  "SELECT * FROM (SELECT * FROM `pantautandonlog` WHERE `objectType` LIKE 'turbidity' AND `nodeId` = '" . $nodeId . "' ORDER BY `timeStamp` DESC LIMIT 12) aTable ORDER BY `timeStamp` ASC";
$dataPointsTurbidity = array();
if ($resultTurbidity = $conn->query($sqlTurbidityChart)) {
  while ($rowTurbidity = $resultTurbidity->fetch_assoc()) {
    array_push($dataPointsTurbidity, array("label" => $rowTurbidity['timeStamp'], "y" => (float)$rowTurbidity['valObject']));
    if ((int)$rowTurbidity['valObject'] >= $valTurbidity) {
...
$sql = "SELECT * FROM `pantautandon` WHERE `objectType` LIKE 'turbidity' AND `nodeId` = '" . $nodeId . "'";
$result = $conn->query($sql);
?>

<table class='w3-table-all w3-centered w3-card w3-white'>
  <tr class='w3-blue'>
    <th style='vertical-align: middle;'>Turbidity (ntu)</th>
  ...
  $(function() {
    var chart = new CanvasJS.Chart("chartContainer", {
      ...
      data: [{
        ...
        color: "#fd7f11",
        showInLegend: true,
        dataPoints: <?php echo json_encode($dataPointsTurbidity); ?>
      }]
    });
    chart.render();
  });
