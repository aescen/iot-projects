...
<?php		
	...
	if( isset($dateSelect) ){
		...
		if (isset($_SESSION['node'])){
			$queryHistory      = "SELECT dcVoltage AS dcv, dcCurrent AS dci, timeStamp AS ts, meshId FROM `nrf24lampulog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `nrf24lampulog`.`timeStamp` DESC";
		} else if (isset($_SESSION['PJU'])){
			$queryHistory      = "SELECT acVoltage AS acv, acCurrent AS aci, timeStamp AS ts, meshId FROM `nrf24lampulog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `nrf24lampulog`.`timeStamp` DESC";
		} else {
			$queryHistory      = "SELECT dcVoltage AS dcv, dcCurrent AS dci, timeStamp AS ts, meshId FROM `nrf24lampulog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `nrf24lampulog`.`timeStamp` DESC";
		}
		...
		if (isset($_SESSION['node'])){
			$queryHistory      = "SELECT dcVoltage AS dcv, dcCurrent AS dci, timeStamp AS ts, meshId FROM `nrf24lampulog` ORDER BY `nrf24lampulog`.`timeStamp` DESC";
		} else if (isset($_SESSION['PJU'])){
			$queryHistory      = "SELECT acVoltage AS acv, acCurrent AS aci, timeStamp AS ts, meshId FROM `nrf24lampulog` ORDER BY `nrf24lampulog`.`timeStamp` DESC";
		} else {
			$queryHistory      = "SELECT dcVoltage AS dcv, dcCurrent AS dci, timeStamp AS ts, meshId FROM `nrf24lampulog` ORDER BY `nrf24lampulog`.`timeStamp` DESC";
			$_SESSION['node'] = 1;
		}
	...
	<body class="w3-light-grey">
		...
		<div class="w3-main w3-light-grey" style="margin-left:140px;margin-top:43px;">
		  ...
		  <header class="w3-container" style="padding-top:22px">
			<div class="w3-container w3-card w3-green w3-cell-row">
				<h2>History</h2>
			</div>
		  </header>
				...
				<div class="w3-cell-row w3-card w3-padding w3-white">
					<div class="w3-cell">
						<div class="w3-left-align">
							<form action="history.php" method="post" onsubmit="return valDate()" required>
								...
								<input class="w3-button w3-ripple w3-green w3-padding-small" type="submit" value="Pilih">
								<input class="w3-button w3-ripple w3-green w3-padding-small" type="submit" value="Reset" id="resetDate" name="resetDate">
							</form>
					...
					<div class="w3-cell">
						<div class="w3-right-align">
							<button onclick="nodeSend();" class="w3-button w3-ripple w3-padding-small <?php echo (isset($_SESSION['node']) ? "w3-gray" : "w3-green");?>" id="node" name="node">Node</button>
							<button onclick="PJUSend();" class="w3-button w3-ripple w3-padding-small <?php echo (isset($_SESSION['PJU']) ? "w3-gray" : "w3-green");?>" id="PJU" name="PJU">PJU</button>
								...
								if (isset($_SESSION['node'])){
									...
									if ($PaginatorHistory->getNumRows() > 0) {
										for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
											echo "<tr>";
												echo "<td>".($i+1)."</td>";
												echo "<td>".$resultsHistory->data[$i]['dcv']."</td>";
												echo "<td>".(!isset($resultsHistory->data[$i]['dci']) ? "-" : $resultsHistory->data[$i]['dci'])."</td>";
												echo "<td>".(!isset($resultsHistory->data[$i]['ts']) ? "-" : $resultsHistory->data[$i]['ts'])."</td>";
												echo "<td>".hexdec($resultsHistory->data[$i]['meshId'])."</td>";
											echo "</tr>";
										endfor; 
									}
									else{
									...
									if ($PaginatorHistory->getNumRows() > 0) {
										for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
											echo "<tr>";
												echo "<td>".($i+1)."</td>";
												echo "<td>".$resultsHistory->data[$i]['acv']."</td>";
                                                echo "<td>".(!isset($resultsHistory->data[$i]['aci']) ? "-" : $resultsHistory->data[$i]['aci'])."</td>";
												echo "<td>".(!isset($resultsHistory->data[$i]['ts']) ? "-" : $resultsHistory->data[$i]['ts'])."</td>";
												echo "<td>".hexdec($resultsHistory->data[$i]['meshId'])."</td>";
											echo "</tr>";
										endfor; 
