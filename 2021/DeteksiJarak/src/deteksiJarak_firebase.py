...
def main():
    ...
    def imageStream(host="localhost", port="5000", debug=False, secure=False):
        ...
        def updateEventDb(log):
            if isFirebase:
                firebaseDb.child("Aisya").child("events").update(log['events'])
        ...
        def saveToDb(log):
            if isFirebase:
                ...
                value = {
                    ...
                    'mask': log['mask'],
                    'no_mask': log['no_mask'],
                    'incorrect_mask': log['incorrect_mask'],
                    'risky_distance': log['risky_distance']
                }
        ...
        def events_stream():
            try:
                while True:
                    msg = json.dumps(streamQueue.get(True)['events'])
        ...
        def apiStream():
            resp = FlaskResponse(events_stream(), mimetype="text/event-stream", content_type="text/event-stream")
    ...
    try:
        ...
        while True:
            ...
            if od.getRiskyDetection() > 0 or od.getNoMaskDetection() > 0  or od.getIncorrectMaskDetection() > 0:
                tts.say(STR_MSG)
...
class TextToSpeech:
    ...
    def say(self, text):
        ...
        if not self.isSpeaking:
            ...
            #print('[INFO] TTS == still speaking. Try again later.')
            pass
...
class ObjectDetection:
    ...
    def personDetection(self, frame):
        ...
        distances = self.getDistanceSimple(midPoints, self.personDetected, frame)
    ...
    def getDistance(self, midPoints, totalPerson, frame=None):
