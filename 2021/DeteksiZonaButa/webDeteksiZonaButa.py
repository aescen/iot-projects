...
def main():
    def runServer(host="192.168.0.2", port="8888", debug=False):
    ...
    try:
        ...
        while(True):
            ...
            if objectCountTmp != objectCount:
                ...
                if int(objectCount) >= 1:
                    t2s.say(str("Jumlah terdeteksi," + str(objectCount) + " orang."))
                    pinon(RELAY_PIN)
                else:
                    pinoff(RELAY_PIN)
...
class Text2Speech:
    def __init__(self, rate=125, volume=1.0):
        ...
        self.engine.setProperty('voice', 'indonesian')
    ...
    def say(self, txt):
        self.txt = txt
        saythread = Thread(target=self.processTxt,
                    name='Text2Speech Say:' + txt, args=())
