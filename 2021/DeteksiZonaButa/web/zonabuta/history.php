...
<?php
	...
	if( isset($dateSelect) ){
		...
		$queryHistory      = "SELECT * FROM `zonabutalog` WHERE `timeStamp` BETWEEN '$startTime' AND '$endTime' ORDER BY `zonabutalog`.`timeStamp` DESC";
		
		$ymd = date("Y-m-d", strtotime($dateSelect));
		$_SESSION['ymd'] = $ymd;
		
	} else{
		$queryHistory      = "SELECT * FROM `zonabutalog` ORDER BY `zonabutalog`.`timeStamp` DESC";
	}
			  ...
			  <div>
				...
				<table class="w3-table-all w3-centered w3-card-4 w3-white">
							<tr class='w3-blue-grey'>
								...
								<th style="vertical-align: middle;">Deteksi</th>
								<th style="vertical-align: middle;">Tipe Objek</th>
                                <th style="vertical-align: middle;">Total Deteksi</th>
								<th style="vertical-align: middle;">Timestamp</th>
							</tr>
							<?php	
								if ($PaginatorHistory->getNumRows() > 0) {
									for( $i = 0; $i < count( $resultsHistory->data ); $i++ ) :
                                        $objectType = json_decode($resultsHistory->data[$i]['objectType'], true);
										echo "<tr>";
											echo "<td>".($i+1)."</td>";
											echo "<td>".$resultsHistory->data[$i]['objectCount']."</td>";
											echo "<td>mobil: ". $objectType['mobil'] .
                                                " | sepeda_motor: " . $objectType['mobil'] .
                                                " | orang: " . $objectType['mobil'] .
                                                "</td>";
											echo "<td>".$resultsHistory->data[$i]['totalDetection']."</td>";
