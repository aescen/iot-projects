...
# Read in ROIs from file#baca koordinat ROI
f = open(sys.argv[1], 'rb')
ROI = pickle.load(f)
f.close()

# Read in image of empty parkinglot#baca gambar kosong
origIMG = cv2.imread(sys.argv[2], 1)

# Read in video file to analyze#baca video
...
for r in ROI:  # proses canny
    (x1, y1, x2, y2) = r
    template = origIMG[y1:y2, x1:x2, :]
    cv2.imwrite('newTemplate' + str(r) + '.jpg', template)
    templateEdgesCanny = cv2.Canny(template, 100, 200)
    templateEdges.append(templateEdgesCanny)
    cv2.imwrite('newCannyTemplate' + str(r) + '.jpg', templateEdgesCanny)
...
while True:
    ...
    if i % 30 == 0:
        ...
        print(len(ROI))
        print(range(0, len(ROI)))
        for spot in range(0, len(ROI)):
            (x1, y1, x2, y2) = ROI[spot]
            new = img[y1:y2, x1:x2, :]
            cv2.imwrite('new' + str(spot) + '.jpg', new)

            # extract Canny edges
            newEdges = cv2.Canny(new, 100, 100)
            cv2.imwrite('newCanny' + str(spot) + '.jpg', newEdges)
            ...
            if sum(sum(normSub)) > threshold:
                if Win:
                    ...
                    cv2.rectangle(imgCanny, (x1, y1), (x2, y2), [0, 0, 255], 2)
                    with open("logs_occupied.log", "a") as logs:
                        print(
                            '[' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ']' + 'Parking spot # ' + str(
                                spot) + ': occupied')
                        logs.write('Parking spot number # ' + str(spot) + ': occupied' + '\n')
                ...
                if Win:
                    ...
                    cv2.rectangle(imgCanny, (x1, y1), (x2, y2), [0, 255, 0], 2)
                    jk = jk + 1
                    with open("logs_vacant.log", "a") as logs:
                        print(
                            '[' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ']' + 'Parking spot # ' + str(
                                spot) + ': vacant')
                        logs.write('Parking spot number # ' + str(spot) + ': vacant' + '\n')
                else:
                    # or print vacant
                    jk = jk + 1
                    print('Parking spot # ' + str(spot) + ': vacant')
            print('Jumlah parkiran kosong ' + str(jk))
        if Win:
            # show the next frame in the image
            cv2.imshow('video', img)
            cv2.imwrite('imageROIs.jpg', img)
            cv2.imshow('video_canny', imgCanny)
            cv2.imwrite('imageROIs_canny.jpg', imgCanny)
            # outvideo.write(img)
        else:
            # clear the screen
            os.system('clear')
